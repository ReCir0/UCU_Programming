...
def board_generation() -> List[list]:
    """
    Generates a game board of 16 x 4 size, i.e. two dimensional list (array) of 'g's, '\
w's and '0's  that is used for the game.
    ...
    for i in range(16):
        ...
        list_of_posibilities = [i - i + j - j, "g", "w"]
...
['w', 'g', 'g', 'g'], ['w', 'w', 'g', 'w'], [0, 0, 0, 'w'], [0, 'w', 'g', 'g'], \
[0, 0, 0, 0], [0, 0, 0, 0], [0, 'g', 'w', 'w'], [0, 0, 'w', 'g'], [0, 0, 'w', 'g']])
    False
...
[(3, 15), (3, 0), (3, 1), (3, 2)]])
    ...
    for i in range(len(board) - 3):
        if board[i][0] == board[i + 1][1] and board[i][0] == board[i + 2][2] \
            ...
            for multip in range(4):
                wining_comb.append((multip, i + multip))
            wining_combinations.append(wining_comb)
            wining_comb = []

    for i in range(len(board) - 3):
        if board[i + 3][0] == board[i + 2][1] and board[i + 3][0] == board[i + 1][2] \
            and board[i + 3][0] == board[i][3] and board[i + 3][0] != 0:
            for multip in range(4):
                wining_comb.append((3 - multip, i + multip))
            wining_combinations.append(wining_comb)
            wining_comb = []
    ...
    if (board[13][0] == board[14][1] and board[13][0] == board[15][2] \
        ...
        for multip in range(3):
            wining_comb.append((multip, 13 + multip))
    ...
    if (board[14][0] == board[15][1] and board[14][0] == board[0][2] \
        ...
        for multip in range(2):
            wining_comb.append((multip, 14 + multip))
        wining_comb.append((j, 0))
        wining_comb.append((j, 1))
        wining_combinations.append(wining_comb)
        wining_comb = []
    if (board[15][0] == board[0][1] and board[15][0] == board[1][2] \
        ...
        for multip in range(3):
            wining_comb.append((multip + 1, multip))
        wining_combinations.append(wining_comb)
        wining_comb = []
    wining_combinations.sort()
